...
def date_to_iso(string):
    ...
    # extract date
    if re.search(r'(\d\d?|' + expressions.ORDINAL_WORDS + r'|' + expressions.ORDINAL_NUMS + r')\s+(' + expressions.MONTHS + r'|' + expressions.MONTH_ABBRS + r'\s*\.?)\s*,?\s+(\d\d(\s|\Z)|\d{4}\b)', string, re.I) != None:
        match = re.search(r'(\d\d?|' + expressions.ORDINAL_WORDS + r'|' + expressions.ORDINAL_NUMS + r')\s+(' + expressions.MONTHS + r'|' + expressions.MONTH_ABBRS + r'\s*\.?)\s*,?\s+(\d\d(\s|\Z)|\d{4}\b)', string, re.I)
        d = ordinal_to_num(match.group(1))
        m = month_to_num(match.group(5))
        y = match.group(7)
        
    elif re.search(r'(' + expressions.MONTHS + r'|' + expressions.MONTH_ABBRS + r'\s*\.?)\s+(\d\d?|' + expressions.ORDINAL_WORDS + r'|' + expressions.ORDINAL_NUMS + r')\b,?\s*(\d\d(\s|\Z)|\d{4}\b)', string, re.I) != None:
        match = re.search(r'(' + expressions.MONTHS + r'|' + expressions.MONTH_ABBRS + r'\s*\.?)\s+(\d\d?|' + expressions.ORDINAL_WORDS + r'|' + expressions.ORDINAL_NUMS + r')\b,?\s*(\d\d(\s|\Z)|\d{4}\b)', string, re.I)
            ...
            if zm != None:
                ...
                if zm.group(1).upper() in timezones:
                    ...
                    if zone < 0:
                        ...
                        zone = '+%02d00' % zone
    elif re.search(r'(\d\d)(\d\d)\s+(h(ou)?rs?|(on\s+)?\d\d?\/\d)', string, re.I) != None:
